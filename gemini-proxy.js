...
exports.handler = async function (event, context) {
    ...
    try {
        ...
        const { prompt, generationConfig, responseSchema, responseMimeType } = JSON.parse(event.body);
        ...
        const payload = {
            ...
            generationConfig: {
                responseMimeType: responseMimeType || "text/plain", // Default para texto puro
            }
        };

        // Adiciona responseSchema se for fornecido
        if (responseSchema) {
            payload.generationConfig.responseSchema = responseSchema;
        }
        if (generationConfig && generationConfig.speechConfig) {
            payload.generationConfig.speechConfig = generationConfig.speechConfig;
            payload.model = generationConfig.model; // Adiciona o modelo para TTS
        }
